...
<p>#<?php e($context->getProduct()->get('number'));
if ($context->getVariation()) {
    e('.'.$context->getVariation()->getNumber());
} e(' '.$context->getProduct()->get('name'));
if ($context->getVariation()) {
    e(' - '.$context->getVariation()->getName());
} ?></p>
...
<fieldset>
    ...
    <div class="formrow">
      <label for="quantity"><?php e(t('Quantity')); ?></label>
        <input type="text" name="quantity" id="quantity" value="<?php if (isset($values['quantity'])) {
            e($values['quantity']);
} ?>" size="3" />
    </div>
    ...
    <div class="formrow">
        ...
        <input type="text" name="description" id="description" value="<?php if (isset($values['description'])) {
...
<input type="hidden" name="product_variation_id" value="<?php if ($context->getVariation()) {
    e($context->getVariation()->getId());
} ?>" />

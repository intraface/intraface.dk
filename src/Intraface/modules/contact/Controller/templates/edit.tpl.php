...
<?php /* removed functionality if (empty($value['id'])): ?>
...
<fieldset>
    ...
    <input type="hidden" name="id" value="<?php if (!empty($value['id'])) {
        e($value['id']);
} ?>" />

    <div class="formrow">
        <label for="number"><?php e(t('Contact number')); ?></label>
        <input type="text" name="number" id="number" value="<?php if (!empty($value['number'])) {
            e($value['number']);
} ?>" />
    </div>
    <div class="formrow">
        <label for="name"><?php e(t('Name')); ?></label>
        <input type="text" name="name" id="name" value="<?php if (!empty($address['name'])) {
            e($address['name']);
} ?>" size="30" />
    </div>
</fieldset>
    ...
    <div class="formrow">
        ...
        <select id="contact-type" name="type_key">
            ...
            <?php foreach ($context->getContactModule()->getSetting('type') as $key => $v) : ?>
                <option value="<?php e($key); ?>"<?php if (isset($value['type_key']) and $value['type_key'] == $key) {
...
<fieldset class="corporate" id="corporate">
    ...
    <div class="formrow">
        ...
        <input type="text" name="cvr" id="cvr" value="<?php if (!empty($address['cvr'])) {
            e($address['cvr']);
} ?>" />
    </div>
</fieldset>
...
<fieldset>
    ...
    <div class="formrow">
        ...
        <textarea name="address" id="address" rows="2" cols="30"><?php if (!empty($address['address'])) {
            e($address['address']);
} ?></textarea>
    </div>
    <div class="formrow">
        <label for="postalcode"><?php e(t('Zip')); ?></label>
        <input type="text" name="postcode" id="postalcode" value="<?php if (!empty($address['postcode'])) {
            e($address['postcode']);
} ?>" />
    </div>
    <div class="formrow">
        <label for="town"><?php e(t('Town')); ?></label>
        <input type="text" name="city" id="town" value="<?php if (!empty($address['city'])) {
            e($address['city']);
} ?>" />
    </div>
    <div class="formrow">
        <label for="country"><?php e(t('Country')); ?></label>
        <input type="text" name="country" id="country" value="<?php if (!empty($address['country'])) {
            e($address['country']);
} ?>" />
    </div>
</fieldset>
...
<fieldset>
    ...
    <div class="formrow">
        ...
        <input type="text" name="email" id="email" value="<?php  if (!empty($address['email'])) {
            e($address['email']);
} ?>" />
    </div>
    <div class="formrow">
        <label for="phone"><?php e(t('Phone')); ?></label>
        <input type="text" name="phone" id="phone" value="<?php if (!empty($address['phone'])) {
            e($address['phone']);
} ?>" />
    </div>
    <div class="fm-optional formrow">
        <label for="website"><?php e(t('Website')); ?></label>
        <input type="text" name="website" id="website" value="<?php if (!empty($address['website'])) {
            e($address['website']);
} ?>" />
    </div>
</fieldset>
...
<fieldset>
    ...
    <div class="formrow">
        ...
        <input type="text" name="delivery_name" id="deliveryname" value="<?php  if (!empty($delivery_address['name'])) {
            e($delivery_address['name']);
} ?>" size="30" />
    </div>

    <div class="formrow">
        <label for="deliveryaddress"><?php e(t('Address')); ?></label>
        <textarea name="delivery_address" id="deliveryaddress" rows="2" cols="30"><?php  if (!empty($delivery_address['address'])) {
            e($delivery_address['address']);
} ?></textarea>
    </div>
    <div class="formrow">
        <label for="deliverypostalcode"><?php e(t('Zip')); ?></label>
        <input type="text" name="delivery_postcode" id="deliverypostcode" value="<?php if (!empty($delivery_address['postcode'])) {
            e($delivery_address['postcode']);
} ?>" />
    </div>
    <div class="formrow">
        <label for="deliverytown"><?php e(t('Town')); ?></label>
        <input type="text" name="delivery_city" id="deliverytown" value="<?php  if (!empty($delivery_address['city'])) {
            e($delivery_address['city']);
} ?>" />
    </div>
    <div class="formrow">
        <label for="deliverycountry"><?php e(t('Country')); ?></label>
        <input type="text" name="delivery_country" id="deliverycountry" value="<?php  if (!empty($delivery_address['country'])) {
    ...
    <div class="formrow">
        ...
        <select name="preferred_invoice" id="preferred-invoice">
            ...
            foreach ($context->getContactModule()->getSetting('preferred_invoice') as $key => $v) {
                ...
                if (!empty($value['type']) and $value['type'] == "private" and $key == 3) {
                    continue;
                }
                ?>
                <option value="<?php e($key); ?>"
                <?php if (isset($value['preferred_invoice']) and $key == $value['preferred_invoice']) {
                    echo ' selected="selected"';
} ?>
                ><?php e(t($v)); ?></option>
                <?php                                                                                                                                                                                                                                                                                                                                         }
...
<fieldset id="invoice-electronic">
    ...
    <div class="formrow">
        ...
        <input type="text" name="ean" id="ean" value="<?php if (!empty($address['ean'])) {
            e($address['ean']);
} ?>" />
    </div>
</fieldset>

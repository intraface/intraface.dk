...
<form action="<?php e(url()); ?>" method="post">
    ...
    <div class="formrow">
      <label for="title">Titel</label>
        <input type="text" name="title" value="<?php if (!empty($value['title'])) {
            e($value['title']);
} ?>" />
  </div>
<?php
...
?>
      <div style="clear: both;">
        ...
        <textarea name="description" cols="90" rows="10"><?php if (!empty($value['description'])) {
            e($value['description']);
} ?></textarea>
    </div>
</fieldset>
<fieldset>
    ...
    <div class="formrow">
      <label for="sender_name"><?php e(t('Sender name')); ?></label>
        <input type="text" name="sender_name" value="<?php if (!empty($value['sender_name'])) {
            e($value['sender_name']);
} ?>" /> (Hvis den er tom, er intranettet afsender)
    </div>
    <div class="formrow">
      <label for="reply_email"><?php e(t('Reply email')); ?></label>
        <input type="text" name="reply_email" value="<?php if (!empty($value['reply_email'])) {
            e($value['reply_email']);
} ?>" /> (Hvis den er tom, er intranettets e-mail-adresse til svar)
    </div>
</fieldset>
<fieldset>
  ...
  <!--
    <div class="formrow">
      <label for="privacy_policy">Privatlivspolitik</label>
        <input type="text" name="privacy_policy" value="<?php if (!empty($value['privacy_policy'])) {
            e($value['privacy_policy']);
} ?>" />
  </div>
-->
    ...
    <div style="clear: both;">
        ...
        <input type="text" name="subscribe_subject" value="<?php if (!empty($value['subscribe_subject'])) {
            e($value['subscribe_subject']);
} ?>" />
    </div>

    <div style="clear: both;">
        <label for="subscribe_message"><?php e(t('Text in the confirmation email')); ?></label><br />
        <textarea name="subscribe_message" cols="90" rows="10"><?php if (!empty($value['subscribe_message'])) {
            e($value['subscribe_message']);
} ?></textarea>
    </div>
    <!--
    <div style="clear: both;">
        <label for="unsubscribe_message">Frameldingsbesked</label><br />
        <textarea name="unsubscribe_message" cols="90" rows="10"><?php if (!empty($value['unsubscribe_message'])) {
            e($value['unsubscribe_message']);
} ?></textarea>
    </div>
    -->

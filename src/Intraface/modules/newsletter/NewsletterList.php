 ...
 * List
 *
 * Skal h�ndtere de forskellige lister, man kan tilmelde sig p� hjemmesiden.
 *
...
class NewsletterList extends Intraface_Standard
{
    ...
    function save($var)
    {
        ...
        if (!isset($var['sender_name'])) {
            $var['sender_name'] = '';
        }
        if (!isset($var['reply_email'])) {
            $var['reply_email'] = '';
        }
        if (!isset($var['description'])) {
            $var['description'] = '';
        }
        if (!isset($var['optin_link'])) {
            $var['optin_link'] = '';
        }
        if (!isset($var['subscribe_subject'])) {
            $var['subscribe_subject'] = '';
        }
        if (!isset($var['subscribe_message'])) {
     ...
     * Deletes
     *
     * TODO B�r kun kunne slette lister hvor der ikke er breve der ikke er sendt
     *
     * @return boolean
     */
    function delete()

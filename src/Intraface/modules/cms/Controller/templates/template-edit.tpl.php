...
<form method="post" action="<?php e(url(null, array($context->subview()))); ?>">
    <input name="id" type="hidden" value="<?php if (!empty($value['id'])) {
        e($value['id']);
} ?>" />
    <input name="site_id" type="hidden" value="<?php if (!empty($value['site_id'])) {
        e($value['site_id']);
} ?>" />
    ...
    <fieldset>
        ...
        <div class="formrow" id="titlerow">
            ...
            <input name="name" type="text" id="name" value="<?php if (!empty($value['name'])) {
                e($value['name']);
} ?>" size="50" maxlength="255" />
        </div>
        <div class="formrow" id="titlerow">
            <label for="identifier"><?php e(t('Identifier')); ?></label>
            <input name="identifier" type="text" id="name" value="<?php if (!empty($value['identifier'])) {
                e($value['identifier']);
} ?>" size="50" maxlength="255" />
        </div>
        ...
        <div class="formrow" id="titlerow">
            ...
            foreach ($page_types as $key => $page_type) : ?>
                <label for="for_page_type_<?php e($key); ?>"><input name="for_page_type[]" type="checkbox" id="for_page_type_<?php e($key); ?>" value="<?php e($key); ?>" <?php if (!empty($value['for_page_type']) && $value['for_page_type'] & $key) {
                    echo 'checked="checked"';
} ?> /><?php e(t($page_type)); ?></label>
            <?php                                                                                                                                                                                                                                                                                                                                     endforeach; ?>
        </div>

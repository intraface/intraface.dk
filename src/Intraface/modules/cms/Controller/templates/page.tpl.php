...
<ul class="options">
    <li><a class="edit" href="<?php e(url('../', array('edit'))); ?>"><?php e(t('edit settings')); ?></a></li>
    <li><a
...
<form method="post" action="<?php e(url()); ?>"
    ...
    if (!empty($error) and is_array($error) and array_key_exists($section->get('id'), $error)) {
        if (!empty($test) and $section->get('type') != $test) {
            echo '</fieldset>'; // Udkommenteret af sune, da den gav problemer. </fieldset> udskrives hver gang ny sektion inds�ttes, derfor kan jeg ikke se hvorfor den skal v�re der, og det bet�d at der kom en </fieldset> for meget
        }        echo '<p class="error">'.$error[$section->get('id')].$test.$section->get('type').'</p>';
    }

    ?> <?php switch ($section->get('type')) {
    case 'shorttext':
        if (!array_key_exists($section->get('id'), $context->error) and !empty($test) and $test != 'shorttext') {
            echo '</fieldset>';
        }
        if ($test != 'shorttext') {
            echo '<fieldset>';

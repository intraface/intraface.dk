...
<form action="<?php e(url(null, array($context->subview()))); ?>" method="POST">
    <fieldset>
        ...
        <div class="formrow">
            ...
            <input type="text" name="name" size="30" value="<?php if (!empty($value['name'])) {
                e($value['name']);
} ?>" />
        </div>
        <div class="formrow">
            <label><?php e(t('website url')); ?></label>
            <input type="text" name="url" size="30" value="<?php if (!empty($value['url'])) {
                e($value['url']);
} ?>" /> <?php e(t('start url with http://')); ?>
        </div>
    </fieldset>

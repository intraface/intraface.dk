        <fieldset>
            ...
            <div class="formrow">
                ...
                <input type="text" name="headline" id="headline" value="<?php if (!empty($value['headline'])) {
                    e($value['headline']);
} ?>" />
            </div>
            <div class="formrow">
                <label for="no_results"><?php e(t('no results text')); ?></label>
                <input type="text" name="no_results_text" id="no_results" value="<?php if (!empty($value['no_results_text'])) {
                    e($value['no_results_text']);
} ?>" />
            </div>
            <div class="formrow">
                <label for="read_more_text"><?php e(t('read more text')); ?></label>
                <input type="text" name="read_more_text" id="read_more_text" value="<?php if (!empty($value['read_more_text'])) {
                    e($value['read_more_text']);
} ?>" />
            </div>
            ...
            <div class="formrow">
                ...
                <select name="show_type" id="show_type_id">
                    <option value="all"<?php if (!empty($value['show_type']) and $value['show_type'] == 'all') {
                        echo ' selected="selected"';
} ?>><?php e(t('all pages')); ?></option>
                    <?php foreach ($element->section->cmspage->getTypes() as $page_type) : ?>
                        <option value="<?php e($page_type); ?>"<?php if (isset($value['show_type']) and $value['show_type'] == $page_type) {
                            echo ' selected="selected"';
} ?>><?php e(t($page_type)); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        <?php
    ...
    ?>
            ...
            <div class="formrow">
                ...
                <input type="text" name="lifetime" id="lifetime" value="<?php if (!empty($value['lifetime'])) {
                    e($value['lifetime']);
} ?>" /> <?php e(t('days')); ?> <?php e(t('(empty is forever)')); ?>
            </div>
            -->

        <div class="radio">
                <input type="radio" id="show_headline_only" name="show" value="only_headline" <?php if (!empty($value['show']) and $value['show'] == 'only_headline') {
                    echo ' checked="checked"';
} ?> />
                 <label for="show_headline_only"><?php e(t('show only headline')); ?></label>
                 <input type="radio" id="show_all_content" name="show" value="description" <?php if (!empty($value['show']) and $value['show'] == 'description') {
                        echo ' checked="checked"';
} ?> />

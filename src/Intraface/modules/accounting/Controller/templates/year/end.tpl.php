...
switch ($context->getYearEnd()->get('step') + 1) :
    ...
    case 5:
        ...
        <table>
            ...
            <tr<?php if ($statement['type'] == 'headline') {
                echo ' class="headline"';
} elseif ($statement['type'] == 'sum') {
    echo ' class="sum"';
} ?>>

                <td><?php e($statement['number']); ?></td>
                <td><?php e($statement['name']); ?></td>
                <td><?php if ($statement['type'] != 'headline') {
                    e(amountToOutput(abs($statement['saldo'])));
} ?></td>
            </tr>
            <?php endforeach; ?>
            </tbody>
    ...
    case 7:
        ?>
            ...
            <legend>Trin 7: Lås året?</legend>
                ...
                <div>
                    <label><input type="radio" name="lock" value="1" <?php if ($context->getYear()->get('locked') == 1) {
                        echo ' checked="checked"';
} ?>/> Lås</label>
                    <label><input type="radio" name="lock" value="0"<?php if ($context->getYear()->get('locked') == 0) {
                        echo ' checked="checked"';
} ?> /> Lås ikke</label>
                </div>
                <input type="submit" name="previous" value="Forrige" />

...
<p><?php e(t('vat')); ?>: <?php e(t($context->getAccount()->get('vat'))); ?> <?php if ($context->getAccount()->get('vat') != 'none') :
?><?php e(number_format($context->getAccount()->get('vat_percent'), 2, ',', '.').'%'); ?><?php
endif; ?></p>
    ...
    <table>
        ...
        <tbody>
            <?php foreach ($posts as $post) {
                $saldo = $saldo + $post['debet'] - $post['credit']; ?>
            <tr>
                <td><?php if (isset($post['dk_date'])) {
                    e($post['dk_date']);
} ?></td>

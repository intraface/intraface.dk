...
<form id="account" action="<?php e(url(), array($context->subview())); ?>" method="post">
    ...
    <fieldset>
        ...
        <div class="formrow">
            ...
            <input type="text" name="number" id="account_number" value="<?php if (!empty($values['number'])) {
                e($values['number']);
} ?>" />
        </div>
        <div class="formrow">
            <label for="account_name">Kontonavn</label>
            <input type="text" name="name" id="account_name" value="<?php if (!empty($values['name'])) {
                e($values['name']);
} ?>" />
        </div>
        <div class="formrow">
            ...
            <select name="type_key" id="account_type">
                ...
                <?php foreach ($context->getAccount()->types as $type_key => $type) { ?>
                    <option value="<?php e($type_key); ?>"<?php if (!empty($values['type_key']) and $type_key == $values['type_key']) {
        ...
        <div class="formrow">
            ...
            <select name="use_key" id="account_usage">
                <?php foreach ($context->getAccount()->use as $use_key => $use) { ?>
                    <option value="<?php e($use_key); ?>"<?php if (!empty($values['use_key']) and $use_key == $values['use_key']) {
                        echo ' selected="selected"';
} ?>><?php e(t($use)); ?></option>
                <?php } ?>
        ...
        <div class="formrow">
            ...
            <select name="vat_key" id="vat_id">
                <?php foreach ($context->getAccount()->vat as $vat_key => $vat) { ?>
                    <option value="<?php e($vat_key); ?>"<?php if (!empty($values['vat_key']) and $vat_key == $values['vat_key']) {
                        echo ' selected="selected"';
} ?>><?php e(t($vat)); ?></option>
                <?php } ?>
            </select>
    ...
    <legend>Summen på kontoen udregnes på følgende konti</legend>
        <div>
            ...
            <input type="text" name="sum_from" id="sum_from" value="<?php if (!empty($values['sum_from'])) {
                e($values['sum_from']);
} ?>" />
            <label for="sum_to">Til kontonummer</label>
            <input type="text" name="sum_to" id="sum_to" value="<?php if (!empty($values['sum_to'])) {
                e($values['sum_to']);
} ?>" />
        </div>
    </fieldset>

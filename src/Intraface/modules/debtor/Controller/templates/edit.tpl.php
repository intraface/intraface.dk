...
<fieldset>
    ...
    <div class="formrow">
        ...
        <input class="input" id="description" name="description" value="<?php if (isset($value["description"])) {
            e($value["description"]);
} ?>" size="60" />
    </div>
    <div class="formrow">
    ...
    <?php if ($context->getDebtor()->get("type") == "invoice") { ?>
        <div class="formrow">
            ...
            <input class="input" type="checkbox" name="round_off" id="round_off" value="1" size="10" <?php if (isset($value["round_off"]) && $value["round_off"] == 1) {
        ...
        <div class="formrow">
            ...
            <select name="currency_id" id="currency_id">
                ...
                foreach ($gateway->findAllWithExchangeRate() as $currency) {
                    ?>
                    <option value="<?php e($currency->getId()); ?>" <?php if (false !== ($debtor_currency = $context->getDebtor()->getCurrency()) && $debtor_currency->getId() == $currency->getId()) {
    ...
    <div class="formrow">
        ...
        <textarea id="message" name="message" cols="80" rows="5"><?php if (isset($value["message"])) {
            e($value["message"]);
} ?></textarea>
    </div>
        ...
        <div class="formrow">
            ...
            <select name="contact_person_id" id="contact_person_id">
                ...
                for ($i = 0, $max = count($persons); $i < $max; $i++) {
                    ?>
                    <option value="<?php e($persons[$i]["id"]); ?>" <?php if (!empty($value["contact_person_id"]) and $value["contact_person_id"] == $persons[$i]["id"]) {
...
<?php if ($context->getDebtor()->get("type") == "invoice" || $context->getDebtor()->get("type") == "order") { ?>
    <fieldset class="radiobuttons">
        ...
        <div>
            <label<?php if (isset($value['payment_method']) && $value['payment_method'] == 0) {
                print(" class=\"selected\"");
} ?>><input class="input" id="none" type="radio" name="payment_method" value="0" <?php if (isset($value['payment_method']) && $value['payment_method'] == 0) {
    print("checked=\"CHECKED\"");
} ?> />
            <?php e(t('None')); ?></label>
        </div>
    <?php if ($context->getKernel()->setting->get('intranet', 'bank_account_number')) { ?>
        <div>
            <label<?php if (isset($value['payment_method']) and $value['payment_method'] == 1) {
                print(' class="selected"');
} ?>><input class="input" id="account" type="radio" name="payment_method" value="1" <?php if (isset($value['payment_method']) and $value['payment_method'] == 1) {
    print("checked=\"CHECKED\"");
} ?> />
    ...
    <?php if ($context->getKernel()->setting->get('intranet', 'giro_account_number')) { ?>
        <div>
            <label for="giro01"<?php if (isset($value['payment_method']) and $value['payment_method'] == 2) {
                print ' class="selected"';
} ?>><input class="input" type="radio" id="giro01" name="payment_method" value="2" <?php if (isset($value['payment_method']) and $value['payment_method'] == 2) {
    print("checked=\"CHECKED\"");
} ?> />
            Girokort +01</label>
        </div>
        <div class="specialcase<?php if (isset($value['payment_method']) and $value['payment_method'] == 3) {
            print(" selected");
} ?>">
            <input class="input" id="giro71" type="radio" name="payment_method" value="3" <?php if (isset($value['payment_method']) and $value['payment_method'] == 3) {
                print("checked=\"CHECKED\"");
} ?> />
            ...
            <label for="girocode" style="display: none;">Girokode</label> <input class="input" name="girocode" id="girocode" value="<?php if (isset($value['girocode'])) {
                e($value['girocode']);
} ?>" size="16" onfocus="if (document.getElementById) document.getElementById('giro71').checked = true;" /> + <?php e($context->getKernel()->setting->get("intranet", "giro_account_number")); ?>&lt;
        </div>
    <?php } ?>
    <?php if ($context->getKernel()->intranet->hasModuleAccess('shop')) : ?>
        <div>
            <label<?php if (isset($value['payment_method']) and $value['payment_method'] == 4) {
                print(' class="selected"');
} ?>><input class="input" id="account" type="radio" name="payment_method" value="4" <?php if (isset($value['payment_method']) and $value['payment_method'] == 4) {
    print("checked=\"CHECKED\"");
} ?> />
            Per efterkrav</label>
        </div>
    ...
    <?php if ($context->getKernel()->intranet->hasModuleAccess('shop')) : ?>
        <div>
            <label<?php if (isset($value['payment_method']) and $value['payment_method'] == 5) {
                print(' class="selected"');
} ?>><input class="input" id="account" type="radio" name="payment_method" value="5" <?php if (isset($value['payment_method']) and $value['payment_method'] == 5) {
    print("checked=\"CHECKED\"");
} ?> />

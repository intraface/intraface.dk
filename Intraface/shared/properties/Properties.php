...
class Properties extends Standard {
	...
	function Properties(& $object, $id = 0) {
		Properties::__construct($object, $id);
	}

	function __construct(& $object, $id = 0) {
		if (!is_object($object)) {
			trigger_error('Properties kræver et object', FATAL);
		}

		/********************** HER ER JEG KOMMET TIL ************************/
	...
	function getConnectedKeywords() {
		...
		$db->query("SELECT distinct(keyword_id), keyword.keyword FROM keyword_x_object x
			INNER JOIN keyword 
			WHERE keyword.type = '".$this->type."' 
				AND keyword.intranet_id = " . $this->object->kernel->intranet->get('id') . "
				AND x.intranet_id =  " . $this->object->kernel->intranet->get('id') . "
				AND x.belong_to = " . $this->object->get('id') . "
				AND keyword.keyword <> '' 
			ORDER BY keyword");
     
		$i = 0;
		while ($db->nextRecord()) {
			$keywords[$i]['id'] = $db->f('keyword_id');
			$keywords[$i]['keyword'] = $db->f('keyword');
			$i++;
		}
		return $keywords;
	...
	function deleteConnectedKeywords() {
		...
		$db = new DB_Sql;
		$db->query("DELETE FROM keyword_x_object 
			WHERE  
 				intranet_id = " . $this->object->kernel->intranet->get('id') . " 
				AND belong_to = " . $this->object->get('id'));
		return 1;
	...
	function getConnectedKeywordsAsString() {
		$db = new DB_Sql();
		$db->query("SELECT * FROM keyword INNER JOIN keyword_x_object x ON keyword.id = x.keyword_id WHERE type='".$this->type."' AND belong_to = " . $this->object->get('id'));
		$keywords = '';
		while ($db->nextRecord()) {
			$keywords .= $db->f('keyword') . ' ';
		}

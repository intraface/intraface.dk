...
<form action="<?php e(url(null)); ?>" method="post">
    <fieldset>
        ...
        <div class="formrow">
            ...
            <input type="text" name="running_index" size="6" value="<?php if (!empty($value['running_index'])) echo safeToForm($value['running_index']); ?>" />
        ...
        <div class="formrow">
            ...
            <select name="evaluate_method_key">
                <?php foreach($settings['evaluate_method'] AS $key => $evaluate_method): ?>
                    <option value="<?php print(intval($key)); ?>" <?php if (!empty($value['evaluate_method_key']) && $value['evaluate_method_key'] == $key) echo 'selected="selected"'; ?> ><?php e(t($evaluate_method)); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div class="formrow">
            ...
            <select name="action_unit_key">
                <?php foreach($settings['action_unit'] AS $key => $action_unit): ?>
                    <option value="<?php print(intval($key)); ?>" <?php if (!empty($value['action_unit_key']) && $value['action_unit_key'] == $key) echo 'selected="selected"'; ?> ><?php e(t($action_unit)); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    </fieldset>

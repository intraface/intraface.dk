...
// man kan ikke slette fx billedteksten
// Der bør laves et tjek på, om værdien er sat overhovedet,
// hvis den er sat, men det nye indhold er tomt, skal den slettes.
...
class CMS_Parameter extends Standard {
	...
	var $loaded = false; // bruges fordi objekterne kan ændre sige
	var $value = array();
	...
	function save($parameter, $value) {
		...
		if ($this->object->get('id') == 0) {
			trigger_error('Parameter::save() object cannot be 0 - problems in ' . get_class($this->object));
		}

		# mangler noget validering - skal sikkert kunne sættes fra elementet?
		...
		// hvis parameterværdien er tom skal den gamle parameter slettes
		elseif (empty($value) AND !empty($old_parameter)) {
	...
	/**
	 * Denne funktion loader alle de nødvendige parametre til et element ind i et array.

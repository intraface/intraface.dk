...
switch ($value['type']) {
	...
	case 'delicious':
		// hvis der er flere bør vi også understøtte dem.
		?>
		<fieldset>
	...
	case 'map':
		...
		<fieldset>
			...
			<div class="formrow">
				...
				<input type="text" value="<?php if (!empty($value['address'])) echo safeToForm($value['address']); ?>" name="address" />
			</div>
		</fieldset>
		<?php
	break;
	...
	case 'gallery':
		...
		<fieldset>
			...
			<!-- Egentlig skulle dette bare være en, og så kan man vælge mellem flickr mv. også måske? -->
			<?php /* if($kernel->user->hasModuleAccess('filemanager')): ?>

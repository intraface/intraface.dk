...
<h1><?php echo safeToHtml($translation->get('pages')); ?> <?php echo safeToHtml($translation->get('on', 'common')); ?> <?php echo $cmssite->get('name'); ?></h1>
...
<?php if (!is_array($pages) OR count($pages) == 0): ?>
<p><?php echo safeToHtml($translation->get('no pages found')); ?></p>
<?php else: ?>
<table>
...
<?php foreach ($pages AS $p):?>
	<tr>
		<td><a href="page.php?id=<?php echo $p['id']; ?>"><?php echo safeToHtml(str_repeat("- ", $p['level']) . $p['title']); ?></a></td>
		...
		<td>		<?php if ($p['status'] == 'published'): // hack siden kan kun vises, hvis den er udgivet. Der bør laves et eller andet, så det er muligt anyways - fx en hemmelig kode på siden ?>
		...
		<td class="options">
			...
			<a class="edit" href="page_edit.php?id=<?php echo $p['id']; ?>"><?php echo safeToHtml($translation->get('edit', 'common')); ?></a>

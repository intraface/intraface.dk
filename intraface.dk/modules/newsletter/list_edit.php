...
<form action="<?php echo basename($_SERVER['PHP_SELF']); ?>" method="post">
	...
	<div class="formrow">
	  <label for="title">Titel</label>
		<input type="text" name="title" value="<?php if (!empty($value['title'])) echo safeToForm($value['title']); ?>" />
  </div>
<?php
/*
	<div class="formrow">
...
?>
  	<div style="clear: both;">
		...
		<textarea name="description" cols="90" rows="10"><?php if(!empty($value['description'])) echo safeToForm($value['description']); ?></textarea>
	</div>
</fieldset>
<fieldset>
	...
	<div class="formrow">
	  <label for="sender_name">Navn på e-mailen</label>
		<input type="text" name="sender_name" value="<?php if (!empty($value['sender_name'])) echo safeToForm($value['sender_name']); ?>" /> (Hvis den er tom, er intranettet afsender)
  </div>
	<div class="formrow">
	  <label for="reply_email">Svar e-mail</label>
		<input type="text" name="reply_email" value="<?php if (!empty($value['reply_email'])) echo safeToForm($value['reply_email']); ?>" /> (Hvis den er tom, er intranettets e-mail-adresse til svar)
  </div>
  </fieldset>
  <fieldset>
  ...
  <!--
	<div class="formrow">
	  <label for="privacy_policy">Privatlivspolitik</label>
		<input type="text" name="privacy_policy" value="<?php if (!empty($value['privacy_policy'])) echo safeToForm($value['privacy_policy']); ?>" />
  </div>
-->

	<div style="clear: both;">
		<label for="subscribe_message">Tekst i e-mailen hvor man skal bekræfte sin tilmelding</label><br />
		<textarea name="subscribe_message" cols="90" rows="10"><?php if (!empty($value['subscribe_message'])) echo safeToForm($value['subscribe_message']); ?></textarea>
	</div>
	<!--
	<div style="clear: both;">
		<label for="unsubscribe_message">Frameldingsbesked</label><br />
		<textarea name="unsubscribe_message" cols="90" rows="10"><?php if (!empty($value['unsubscribe_message'])) echo safeToForm($value['unsubscribe_message']); ?></textarea>
	</div>
	-->

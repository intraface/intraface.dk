...
<?php if($contact_id): ?>
	<ul class="options">
		<li><a href="reminder_edit.php?contact_id=<?php print(intval($contact_id)); ?>">Opret rykker</a></li>
		<li><a href="/modules/contact/contact.php?id=<?php echo intval($contact->get('id')); ?>">Gå til kontakten</a>
	</ul>
<?php endif; ?>
...
<tbody>
	...
	for($i = 0, $n = count($reminders); $i < $n; $i++) {
		?>
		<tr id="i<?php print(safeToHtml($reminders[$i]["id"])); ?>"<?php if(isset($_GET['id']) && $_GET['id'] == $reminders[$i]['id']) print(" class=\"fade\""); ?>>
			<td class="number"><?php print(safeToHtml($reminders[$i]["number"])); ?></td>
			<td><a href="reminders.php?contact_id=<?php print(intval($reminders[$i]["contact_id"])); ?>"><?php print(safeToHtml($reminders[$i]["name"])); ?></a></td>
			<td><a href="reminder.php?id=<?php print(intval($reminders[$i]["id"])); ?>"><?php (trim($reminders[$i]["description"] != "")) ? print(safeToHtml($reminders[$i]["description"])) : print("[Ingen beskrivelse]"); ?></a></td>
			<td class="date">
				<?php
				if ($reminders[$i]["status"] != "created") {
					print(safeToHtml($reminders[$i]["dk_date_sent"]));
				}
				else {
					print("Nej");
				}
			...
			<td><?php print(safeToHtml($reminders[$i]["send_as"])); ?></td>
			<td class="date">
				<?php
				if($reminders[$i]["status"] == "executed" || $reminders[$i]["status"] == "canceled") {
					print(safeToHtml($reminders[$i]["status"]));
				}
				elseif($reminders[$i]["due_date"] < date("Y-m-d")) {
					print("<span class=\"red\">".safeToHtml($reminders[$i]["dk_due_date"])."</span>");
				}
				else {
					print(safeToHtml($reminders[$i]["dk_due_date"]));
				}
				?>
			</td>
			<td class="buttons">
				...
				if($reminders[$i]["locked"] == 0) {
					?>
					<a class="edit" href="reminder_edit.php?id=<?php print(intval($reminders[$i]["id"])); ?>">Ret</a>
					<?php if ($reminders[$i]["status"] == "created"): ?>
					<a class="delete" href="reminders.php?contact_id=<?php print(intval(intval($_GET["contact_id"]))); ?>&amp;delete=<?php print(intval($reminders[$i]["id"])); ?>">Slet</a>
					<?php endif; ?>
					<?php
				}
				?>

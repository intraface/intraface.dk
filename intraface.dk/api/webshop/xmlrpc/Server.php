...
require_once('../../XMLRPC/Server.php');
 
class API_Webshop_XMLRPC_Server extends API_XMLRPC_Server {
	...
	function __construct() {
		...
		*/
		$this->addCallback(
			...
			'Changes a product in basket. Removes a product to the basket. Takes three arguments, <var>struct $credentials</var>, <var>int $product_id</var> and <var>int $change</var>. Returns error -100 if no more is on stock.'
		);
		$this->addCallback(
			'basket.totalPrice',
			'this:getTotalPrice',
...
if($_SERVER['REQUEST_METHOD'] != 'POST' || $_SERVER['CONTENT_TYPE'] == 'application/x-www-form-urlencoded') {
	...
	$server = new API_Webshop_XMLRPC_Server();
}
?>